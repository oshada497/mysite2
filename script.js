...
async function loadPDFPage(page) {
    try {
        ...
        allPdfData = allPdfData.concat(pageData); // Add page data to global array
        return pageData;
    } catch (error) {
...
async function performSearch(query) {
    ...
    const results = [];
    let currentSearchId = Date.now(); // Unique ID for this search
    let searching = true;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function createPdfCard(pdf, index) {
        ...
        card.innerHTML = `
            ...
            <button class="download-btn" data-link="${pdf.link}">
                <i class="fas fa-download"></i>
                Download
            </button>
        `;
        ...
        downloadBtn.addEventListener('click', function(e) {
            ...
            setTimeout(() => {
                window.location.href = `book.html?link=${encodeURIComponent(link)}`;
            }, 2000);
        });
